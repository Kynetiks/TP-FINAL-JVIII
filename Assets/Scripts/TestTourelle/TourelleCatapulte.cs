...
public class TourelleCatapulte : MonoBehaviour
{
    ...
    // Tourner la tête de la tourelle vers la cible (seule la tête bouge, rotation sur l'axe Y uniquement)
    void RotateHeadTowardsTarget()
    {
        ...
        // Garder seulement la rotation sur l'axe Y
        directionToTarget.y = 0;  // Annuler toute composante verticale pour limiter la rotation à l'axe horizontal

        if (directionToTarget.magnitude > 0.1f)  // Éviter la rotation excessive si la cible est proche
        {
            Quaternion targetRotation = Quaternion.LookRotation(directionToTarget);  // Créer la rotation qui va de la tourelle vers la cible
